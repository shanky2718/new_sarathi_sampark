...
import { Truck, MapPin, Navigation, Info, X } from 'lucide-react';
...
export const SimulatedMap: React.FC<SimulatedMapProps> = ({ activeTruckId, onTruckSelect }) => {
  // Cities coordinates on our custom 800x450 canvas grid
  const cities = [
  ...
  // Primary trade route lines connecting cities
  const routeLines = [
  ...
  // In-flight trucks (with updating coordinates based on simulated timers)
  ...
  // Micro-simulation: slowly move truck coordinate dots along paths
  useEffect(() => {
    const timer = setInterval(() => {
      setTrucks(prev => prev.map(t => {
        let newProgress = t.progress + 0.2;
        if (newProgress >= 100) newProgress = 10; // Wrap around for simulation
        
        // Linear interpolation logic between origin and destination coordinates
  ...
  return (
    <div className="relative w-full rounded-xl border border-charcoal/10 bg-[#EFECE6] p-2 shadow-inner overflow-hidden aspect-[16/9]">
      ...
      >
        ...
        {/* Abstract road highway grid network */}
        ...
        {trucks.map((t) => {
          ...
          return (
            ...
            >
              {/* Pulse animation for selected or delayed */}
              <circle
                cx={t.coords.x}
                cy={t.coords.y}
                r={isSelected ? "14" : "9"}
