...
class LocalDB {
  ...
  acceptLoad(loadId: string, truckId: string): { success: boolean; trip?: Trip; load?: ReturnLoad } {
    ...
    this.addNotification(
      'success',
      `Return Load ${load.loadId} (${load.pickup} → ${load.destination}) ACCEPTED! Assigned to Truck ${truckId}.`
    );
  ...
  addFuelEntry(entry: { truckId: string; liters: number; cost: number; mileage: number; driver: string }): FuelMetric {
    ...
    this.addNotification('info', `Fuel refill logged for ${entry.truckId}: ${entry.liters}L.`);
    return newMetric;
  }
  ...
  addMaintenance(record: Omit<MaintenanceRecord, 'id' | 'status'>): MaintenanceRecord {
    ...
    this.addNotification('info', `Maintenance scheduled for Truck ${record.truckId}.`);
    return newRecord;
  }
